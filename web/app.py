...
echo "Opening rendered summary..."
open "http://localhost:8000/research-summary"
...
SUMMARY_PATH = "/tmp/phdata-research-summary.md"

@app.get("/api/cart/summary")
def cart_summary():
    """Return the rendered research summary if it exists."""
    if not os.path.exists(SUMMARY_PATH):
        return JSONResponse(status_code=404, content={"error": "No summary found. Run checkout first."})
...
def research_summary_page():
    """Serve a standalone HTML page that renders the markdown summary with GitHub styling."""
    html = """<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Research Summary — phData Experience</title>
  ...
  <style>
    ...
    .header-btn.primary:hover { background: #2a8a9e; }
    .container {
      max-width: 980px;
      margin: 32px auto;
      padding: 48px;
    ...
    .status h2 { color: #1f2328; margin-bottom: 8px; }
    .spinner {
      width: 32px;
      height: 32px;
      border: 3px solid #d0d7de;
      border-top-color: #3ba3b8;
      border-radius: 50%;
...
<body>
  <div class="header">
    <div>
      <div class="header-title">Research Summary</div>
      <div class="header-subtitle">Generated by phData Experience Portal</div>
    </div>
    <div class="header-actions">
      <a href="/" class="header-btn">Back to Portal</a>
      <button class="header-btn primary" onclick="location.reload()">Refresh</button>
    </div>
  </div>
  <div class="container">
    <div id="content" class="markdown-body">
      <div class="status">
        ...
        <p>Checking for research output</p>
      </div>
    </div>
  </div>
  ...
  <script>
    async function loadSummary() {
      ...
      try {
        ...
        if (res.status === 404) {
          el.innerHTML = `
            <div class="status">
              ...
              <p>The research is still running in Terminal, or hasn't been started yet.</p>
              <p style="margin-top:16px">
                <button class="header-btn primary" onclick="location.reload()" style="color:white">Refresh</button>
              </p>
            </div>`;
        ...
        el.innerHTML = `<div class="status"><h2>Error loading summary</h2><p>${err.message}</p></div>`;
      }
    }
    loadSummary();
  </script>
